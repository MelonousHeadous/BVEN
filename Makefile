...
pkgdatadir = $(datadir)/bitcoinvenezuelacore
pkgincludedir = $(includedir)/bitcoinvenezuelacore
pkglibdir = $(libdir)/bitcoinvenezuelacore
pkglibexecdir = $(libexecdir)/bitcoinvenezuelacore
...
CONFIG_HEADER = $(top_builddir)/src/config/bitcoinvenezuela-config.h
CONFIG_CLEAN_FILES = libbitcoinvenezuelaconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libbitcoinvenezuelaconsensus.pc.in \
	...
	$(top_srcdir)/src/config/bitcoinvenezuela-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/BitcoinVenezuela/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/BitcoinVenezuela/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/BitcoinVenezuela/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/BitcoinVenezuela/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = bitcoinvenezuela-cli
BITCOIN_DAEMON_NAME = bitcoinvenezuelad
BITCOIN_GUI_NAME = bitcoinvenezuela-qt
BITCOIN_TX_NAME = bitcoinvenezuela-tx
...
COPYRIGHT_HOLDERS_FINAL = The BitcoinVenezuela Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = BitcoinVenezuela Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/BitcoinVenezuela/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = bitcoinvenezuelacore
PACKAGE_BUGREPORT = https://github.com/bitcoinvenezuelapay/bitcoinvenezuela/issues
PACKAGE_NAME = BitcoinVenezuela Core
PACKAGE_STRING = BitcoinVenezuela Core 1.0.0
PACKAGE_TARNAME = bitcoinvenezuelacore
PACKAGE_URL = https://bitcoinvenezuela.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/BitcoinVenezuela
abs_srcdir = /tmp/BitcoinVenezuela
abs_top_builddir = /tmp/BitcoinVenezuela
abs_top_srcdir = /tmp/BitcoinVenezuela
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/BitcoinVenezuela/build-aux/install-sh
...
pkgconfig_DATA = libbitcoinvenezuelaconsensus.pc
...
empty := 
space := $(empty) $(empty)
OSX_APP = BitcoinVenezuela-Qt.app
...
DIST_CONTRIB = $(top_srcdir)/contrib/bitcoinvenezuela-cli.bash-completion \
	       $(top_srcdir)/contrib/bitcoinvenezuela-tx.bash-completion \
	       $(top_srcdir)/contrib/bitcoinvenezuelad.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_bitcoinvenezuela_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_bitcoinvenezuela_coverage.info test_bitcoinvenezuela.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/BitcoinVenezuela-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/bitcoinvenezuela-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bitcoinvenezuela-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bitcoinvenezuela-config.h
$(top_srcdir)/src/config/bitcoinvenezuela-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bitcoinvenezuela-config.h src/config/stamp-h1
libbitcoinvenezuelaconsensus.pc: $(top_builddir)/config.status $(srcdir)/libbitcoinvenezuelaconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/BitcoinVenezuela-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitcoinVenezuela-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitcoinVenezuela-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_bitcoinvenezuela.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_bitcoinvenezuela -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_bitcoinvenezuela_filtered.info: test_bitcoinvenezuela.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_bitcoinvenezuela_filtered.info
...
#test_bitcoinvenezuela_coverage.info: baseline_filtered_combined.info test_bitcoinvenezuela_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitcoinvenezuela_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_bitcoinvenezuela_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitcoinvenezuela_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitcoinvenezuela.coverage/.dirstamp:  test_bitcoinvenezuela_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_bitcoinvenezuela.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_bitcoinvenezuela.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
