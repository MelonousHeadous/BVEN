...
//BitcoinVenezuela only features
...
const char * const BITCOIN_CONF_FILENAME = "bitcoinvenezuela.conf";
const char * const BITCOIN_PID_FILENAME = "bitcoinvenezuelad.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitcoinvenezuela" is a composite category enabling all BitcoinVenezuela-related debug output
                if(ptrCategory->count(std::string("bitcoinvenezuela"))) {
...
#else
    const char* pszModule = "bitcoinvenezuela";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinVenezuela
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinVenezuela
    // Mac: ~/Library/Application Support/BitcoinVenezuela
    // Unix: ~/.bitcoinvenezuela
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinVenezuela";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinVenezuela";
#else
    // Unix
    return pathRet / ".bitcoinvenezuela";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitcoinvenezuela.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitcoinvenezuela.conf
